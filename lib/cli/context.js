...
async function buildContext(cwd, args, flags) {
  ...
  context.models = await loadModelClasses(cwd, paths.models, orm);
  return context;
}
...
async function loadModelClasses(cwd, modelsPath, orm) {
  ...
  return await orm.load(appModels);
}
...
function initORM(cwd, conf) {
  ...
  return new ormIdentity.ORM(conf || {orm: {dialect: 'sqlite', storage: ':memory:', logging: false}});
}
