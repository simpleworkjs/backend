...
const {authMiddleware, requireModelPermission, requireInstancePermission, rebuildAccessPolicy} = require('@simpleworkjs/orm-identity').auth;
...
// Parse an integer query param, falling back to `def`, clamped to [min, max].
...
function makeRoutes(Model) {
  ...
  router.get('/', requireModelPermission(Model, 'read'), async function(req, res, next) {
    try {
      ...
      const pageSize = clampInt(req.query.pageSize, Model.pageSize || 20, 1, 500);
      const page = Math.max(1, parseInt(req.query.page, 10) || 1);
      const offset = (page - 1) * pageSize;

      // DB-level pagination (works for the Sequelize adapter; other adapters
      // that don't honour limit/offset degrade to returning the first page's
      // worth from the full set).
      const [rows, total] = await Promise.all([
        Model.list({where, limit: pageSize, offset, order: [[pkName, 'ASC']]}),
        Model.count({where}),
      ]);
      ...
      // owner-scoped model would leak every user's rows to every user. (For
      // owner-scoped models `total` counts all rows and a page may come back
      // partially filtered — an accepted approximation; all-readable models
      // are exact. Security is unaffected: unauthorized rows are still removed.)
      const user = reqPermissionUser(req);
      const results = rows.filter(instance => instance.hasPermission(user, 'read'));
...
function mount(app, models, prefix) {
  ...
  mountAccessRoutes(app, models, prefix);
}

// Read/edit a model's tiered access grants (the permission editor). GET is open
// to any authenticated user (the modal shows it); PUT is admin-only and writes
// the model's governing Role, which triggers a policy rebuild.
function mountAccessRoutes(app, models, prefix) {
  ...
  function normalizeGrants(input) {
    ...
    for (const tier of TIERS) {
      out[tier] = {};
      const t = input[tier] || {};
      for (const action of ACTIONS) out[tier][action] = t[action] === true;
    }
    return out;
  }

  app.get(`${prefix}/_access/:model`, async function(req, res, next) {
    try {
      const Model = models[req.params.model];
      if (!Model) return res.status(404).json({error: {message: 'Unknown model'}});
      res.json({model: Model.name, access: Model.accessSchema()});
    } catch (error) {
      next(error);
    }
  });

  app.put(`${prefix}/_access/:model`, async function(req, res, next) {
    try {
      if (!(req.permissions && req.permissions.has('admin'))) {
        return res.status(req.user ? 403 : 401)
          .json({error: {message: req.user ? 'Admin required' : 'Authentication required'}});
      }
      const Model = models[req.params.model];
      if (!Model) return res.status(404).json({error: {message: 'Unknown model'}});
      const Role = models.Role;
      if (!Role) return res.status(500).json({error: {message: 'Role model not loaded'}});

      const grants = normalizeGrants(req.body);
      const existing = await Role.list({where: {entityModel: Model.name}});
      if (existing.length) {
        await existing[0].update({entityPermissions: grants});
      } else {
        await Role.create({
          name: `${Model.name} access`,
          description: `Access policy for ${Model.name}`,
          entityModel: Model.name,
          entityPermissions: grants,
          isActive: true,
        });
      }
      // Rebuild the policy immediately so the change takes effect on the very
      // next request (the pubsub subscription also rebuilds, but async).
      if (Model.orm) await rebuildAccessPolicy(Model.orm, models);
      res.json({model: Model.name, access: grants});
    } catch (error) {
      next(error);
    }
  });
