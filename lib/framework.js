...
function createSimpleWorkJS(options) {
  ...
  async function init() {
    ...
    // ActivityEvent/ActivitySeen back the notification feed (lib/activity.js).
    // Registered here so an app gets notifications without declaring anything.
    models = await orm.load([
      Object.values(ormIdentity.identity),
      [activity.ActivityEvent, activity.ActivitySeen],
      appModels,
    ]);
    ...
    const apiPrefix = conf.api && conf.api.prefix || '/api';
    modelRoute.mount(app, models, apiPrefix);
    // GET <prefix>/activity + PUT <prefix>/activity/seen. Mounted after
    // modelRoute so it sits behind the same auth middleware.
    activity.mountFeed(app, models, apiPrefix, conf.activity);
  ...
  function attachSockets() {
    ...
    bridgeSub = pubsub.subscribe(/^model:/, function(data) {
      ...
      // Not awaited: a model write must not wait on its history row.
      activity.record(models, data);
